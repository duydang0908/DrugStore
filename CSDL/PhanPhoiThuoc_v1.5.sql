...
(
	...
	MaKH int,
	NgayLapDon date,	
	MaTinhTrang int foreign key references TinhTrangDonHang(MaTinhTrang),
)
...
insert into TinhTrangDonHang values (N'Đang xác nhận')
insert into TinhTrangDonHang values (N'Đã xác nhận')
insert into TinhTrangDonHang values (N'Giao hàng')
insert into TinhTrangDonHang values (N'Đã giao hàng')
...
Go
create TRIGGER HuyDonHang ON CHI_TIET_DON_DAT_HANG FOR DELETE AS 
BEGIN
	UPDATE SAN_PHAM
	SET SoLuongTon = SoLuongTon + (SELECT SoLuong FROM deleted WHERE MaSP = SAN_PHAM.MaSP)
	FROM SAN_PHAM 
...
delete CHI_TIET_DON_DAT_HANG where MaDDH = 9
